...
exports.submitReview = async (req, res) => {
  try {
    ...
    if (project.status !== 'completed') return res.status(400).json({ error: 'Reviews only for completed projects' });
    ...
    const review = new Review({ project: projectId, client: req.user._id, rating, title, body });
    await review.save();
...
exports.moderateReview = async (req, res) => {
  try {
    ...
    const review = await Review.findByIdAndUpdate(
      req.params.id,
      { status, displayOnHomepage: status === 'approved' ? (displayOnHomepage !== false) : false, reviewedBy: req.user._id, reviewedAt: new Date() },
      { new: true }
